...
이 스크립트는 크게 네 부분으로 나누어 읽으면 된다.

| 구간 | 역할 |
|---|---|
| 설정값 선언 | 앱 경로, 포트, 로그 파일, 임계값을 변수로 정한다 |
| 함수 정의 | 프로세스 확인, 포트 확인, 자원 수집 같은 작은 작업을 함수로 나눈다 |
| `main()` 함수 | 위에서 만든 함수들을 실제 실행 순서대로 호출한다 |
| `main "$@"` | 스크립트가 실행될 때 `main()`을 시작한다 |

중요한 점은 Bash에서 함수는 “정의만 한다고 바로 실행되지 않는다”는 것이다.  
예를 들어 `check_port() { ... }`는 포트 확인 방법을 미리 적어 둔 것이고, 실제 실행은 `main()` 안의 `check_port || exit 1` 줄에서 일어난다.
...
첫 줄 `#!/usr/bin/env bash`는 이 파일을 Bash로 실행하라는 뜻이다.  
이 줄을 shebang이라고 부른다. 터미널에서 `/home/agent-admin/agent-app/bin/monitor.sh`처럼 파일 자체를 실행하면 운영체제가 이 줄을 보고 Bash를 찾아 실행한다.

`set -u`는 정의되지 않은 변수를 실수로 사용했을 때 오류로 처리한다.  
예를 들어 변수 이름을 `LOG_FILE`로 써야 하는데 `LOG_FIEL`처럼 오타를 내면, 빈 값으로 조용히 넘어가지 않고 바로 문제를 드러낸다. 운영 스크립트에서는 이런 “조용한 실패”가 위험하므로 초기에 잡는 편이 좋다.
...
여기서 `${변수:-기본값}` 문법은 “변수가 설정되어 있으면 그 값을 쓰고, 없으면 기본값을 쓰라”는 뜻이다.

| 표현 | 의미 |
|---|---|
| `${AGENT_HOME:-/home/agent-admin/agent-app}` | `AGENT_HOME`이 있으면 사용하고, 없으면 `/home/agent-admin/agent-app` 사용 |
| `${AGENT_PORT:-15034}` | `AGENT_PORT`가 있으면 사용하고, 없으면 `15034` 사용 |
| `${AGENT_LOG_DIR:-/var/log/agent-app}` | `AGENT_LOG_DIR`가 있으면 사용하고, 없으면 `/var/log/agent-app` 사용 |

이렇게 작성하면 cron처럼 환경 변수가 적은 곳에서 실행되더라도 기본값으로 동작할 수 있다.  
동시에 나중에 포트나 경로가 바뀌면 스크립트 내부 곳곳을 고치는 대신 환경 변수만 바꿔 실행할 수 있다.
...
`$(( ... ))`는 Bash의 산술 계산 문법이다.  
`10 * 1024 * 1024`는 10MB를 바이트 단위로 표현한 값이다. 즉 `monitor.log`가 10MB 이상 커지면 회전 대상으로 본다.
...
이 함수는 화면에 제목을 출력하는 일만 한다.  
작은 함수로 나누면 `main()`을 읽을 때 전체 흐름이 잘 보인다.
...
이런 식으로 `main()` 안에서 함수 이름을 쓰면 그 함수가 실행된다.
...
`local`은 함수 안에서만 사용할 변수를 만든다.  
`log_file="$1"`에서 `$1`은 이 함수에 전달된 첫 번째 인자다. 아래처럼 호출하면:

```bash
rotate_log_if_needed "$LOG_FILE"
```

함수 안에서 `$1`은 `/var/log/agent-app/monitor.log`가 된다.

로그 회전 흐름은 다음 순서다.

1. 로그 디렉토리가 없으면 만든다.
2. 로그 파일이 없으면 빈 파일로 만든다.
3. 현재 로그 파일 크기를 확인한다.
4. 10MB보다 작으면 아무것도 하지 않고 돌아간다.
5. 10MB 이상이면 기존 `monitor.log.1`, `monitor.log.2` 같은 파일 번호를 뒤로 민다.
6. 현재 `monitor.log`를 `monitor.log.1`로 바꾼다.
...
`pgrep -f -- "$APP_PATTERN"`은 실행 중인 프로세스의 전체 명령줄에서 앱 실행 경로를 찾는다.  
`|`는 파이프라고 부르며, 왼쪽 명령의 출력을 오른쪽 명령의 입력으로 넘긴다.

```text
pgrep 결과 여러 줄 -> head -n 1 -> 첫 번째 PID만 선택
```

이전처럼 단순히 `agent-app`만 찾으면 `monitor.sh` 경로 자체가 같이 잡힐 수 있다.  
그래서 `APP_PATTERN`을 `/home/agent-admin/agent-app/agent-app`처럼 실제 앱 실행 파일 경로로 잡아 오탐을 줄였다.

```bash
check_process "$pid" || exit 1
```

이 줄은 “프로세스 확인에 실패하면 스크립트 전체를 실패로 끝내라”는 뜻이다.  
`||`는 왼쪽 명령이 실패했을 때만 오른쪽 명령을 실행한다.
...
이 줄은 세 명령을 이어 붙인 것이다.

| 부분 | 의미 |
|---|---|
| `ss -tuln` | TCP/UDP LISTEN 포트 목록을 출력한다 |
| `awk '{print $5}'` | 각 줄에서 로컬 주소 칸만 뽑는다 |
| `grep -Eq "(:|\\.)${APP_PORT}$"` | 주소 끝이 `:15034` 또는 `.15034` 형태인지 검사한다 |

`if 명령; then` 구조에서 Bash는 명령의 출력 내용보다 종료 코드를 본다.  
`grep`이 포트를 찾으면 성공 코드 `0`을 반환하고, 찾지 못하면 실패 코드 `1`을 반환한다. 그래서 포트가 열려 있으면 `[OK]`, 아니면 `[FAIL]`이 출력된다.
...
`command -v ufw`는 `ufw` 명령이 설치되어 있는지 확인한다.  
뒤의 `>/dev/null 2>&1`은 정상 출력과 에러 출력을 화면에 보이지 않게 버리는 표현이다. 여기서는 “명령이 있는지 없는지”만 중요하고, 출력 경로는 필요 없기 때문이다.

방화벽은 꺼져 있어도 스크립트를 실패로 종료하지 않는다.  
요구사항에서 방화벽은 경고 대상이고, 앱 프로세스와 포트는 실패 대상이기 때문이다.
...
`free` 출력의 `Mem:` 줄에서 전체 메모리와 사용 중인 메모리를 읽고, 사용률을 계산한다.  
`printf "%.1f"`는 소수점 한 자리까지 출력하라는 뜻이다.
...
`df /`는 루트 파일시스템의 디스크 사용량을 보여 준다.  
`NR==2`는 두 번째 줄만 보겠다는 뜻이고, `gsub(/%/, "", $5)`는 `23%` 같은 값에서 `%` 문자를 제거한다.

CPU는 `top -bn1`의 출력 형식에서 idle 값을 읽고 `100 - idle`로 계산한다.  
idle은 CPU가 놀고 있는 비율이므로, 전체 100에서 idle을 빼면 사용 중인 비율이 된다.
...
이 함수는 이름, 현재값, 기준값, 단위를 받아서 현재값이 기준값보다 크면 경고를 출력한다.

```bash
if awk "BEGIN { exit !($value > $threshold) }"; then
```

Bash 자체는 소수점 비교에 약하다.  
CPU 사용률처럼 `25.3` 같은 소수점을 비교해야 하므로 `awk`를 이용해 비교한다. 조건이 참이면 경고 문장을 출력하고, 거짓이면 아무것도 출력하지 않는다.

#### main 함수의 실행 흐름

`main()`은 스크립트의 실제 실행 순서를 보여 주는 목차와 같다.
...
4. 포트 확인
5. 방화벽 확인
6. CPU/MEM/DISK 사용률 수집
7. 화면에 결과 출력
8. 임계값 초과 경고 출력
9. 로그 파일 크기 확인 및 회전
10. monitor.log에 한 줄 추가
```
...
앱 프로세스가 없거나 포트가 열려 있지 않으면 이후 자원 수집을 해도 의미가 작다.  
그래서 즉시 `exit 1`로 종료해 cron이나 운영자가 실패를 알아차릴 수 있게 한다.

마지막 로그 기록은 다음 줄에서 일어난다.
...
`>> "$LOG_FILE"`은 파일을 덮어쓰지 않고 뒤에 한 줄을 추가한다.  
그래서 `monitor.sh`가 실행될 때마다 `/var/log/agent-app/monitor.log`에 기록이 누적된다.

### 8.5 Bash 문법 미니 사전

`monitor.sh`를 읽을 때 자주 만나는 Bash 문법을 정리하면 다음과 같다.

| 문법 | 의미 |
|---|---|
| `"$변수"` | 변수 값을 문자열로 안전하게 사용한다 |
| `$1`, `$2` | 함수나 스크립트에 전달된 첫 번째, 두 번째 인자 |
| `local name` | 함수 안에서만 쓰는 지역 변수 선언 |
| `$(명령)` | 명령 실행 결과를 변수에 담는다 |
| `$((계산))` | 정수 계산을 수행한다 |
| `명령1 | 명령2` | 명령1의 출력을 명령2로 넘긴다 |
| `명령 || exit 1` | 명령이 실패하면 스크립트를 실패로 종료한다 |
| `>/dev/null 2>&1` | 정상 출력과 에러 출력을 화면에 보이지 않게 버린다 |
| `>> 파일` | 파일 뒤에 내용을 추가한다 |
| `return 0` | 함수가 성공했음을 의미한다 |
| `return 1` | 함수가 실패했음을 의미한다 |
| `exit 1` | 스크립트 전체를 실패 상태로 종료한다 |

초보 단계에서는 모든 문법을 외우기보다 “이 줄이 어떤 운영 질문에 답하는가”를 먼저 보면 된다.  
예를 들어 `check_port`는 “앱 포트가 열려 있는가”, `get_mem_usage`는 “메모리를 얼마나 쓰고 있는가”, `rotate_log_if_needed`는 “로그 파일이 너무 커졌는가”라는 질문에 답한다.
